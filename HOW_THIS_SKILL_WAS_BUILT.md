...
3. **Every failure becomes a guardrail, not a memory.** Painful discoveries were
   promoted, in order of strength: gotcha note, then symptom-indexed reference
   (`references/gotchas.md`), then lint rule (`lint_jrxml.ps1`), then hard gate inside
   the deploy script itself. The strict-Jackson traps that JRS reports as an opaque
...
5. **Regression protection scales with the surface.** As capabilities accumulated, a
   ...
   `/reports/_smoke` folder. Offline prechecks (doc-consistency plus a Pester unit
   suite) run first so a broken script never reaches the server.
...
| JRS's SQL security validator rejects any report query not beginning with `SELECT` (CTEs fail with `JSSecurityException`) | SQL lint blocks a leading `WITH` at deploy time; the documented fix is rewriting CTEs as nested subqueries |
...
| Build span | June 1 to July 10, 2026, across ~50 commits |
...
3. Convert every failure into the strongest guardrail you can afford: note, then
   reference, then lint rule, then hard gate.
