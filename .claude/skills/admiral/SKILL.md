...
name: admiral
description: >-
  Manage Actian Data Platform resources via the Admiral REST API (admiral.aop-stage.aws.actiandatacloud.com).
  Use when the user wants to: list, create, start, stop, sleep, scale, modify, or delete warehouses or
  databases; manage IP allowlists, WLM settings, idle-stop policies, Data API, ML services, external table
...
Automates management of **Actian Avalanche** warehouses and databases against the Admiral
REST API at `https://admiral.aop-stage.aws.actiandatacloud.com/api/v1`.

## Setup

1. Copy the config template and fill in the real password:
   ```powershell
   Copy-Item "$skill\..\admiral.config.example.json" "$skill\..\admiral.config.json"
   # Edit admiral.config.json — set "password" to the real value
   ```
   The real credentials: username `robert.gorsuch@actian.com.idp`, password in your secrets.

2. The config file is `.gitignore`d — it will not be committed.

3. `$skill` in examples below refers to the `scripts/` subdirectory of this skill:
   ```powershell
...
The scripts use OAuth2 password flow (`POST /login`) automatically. The token (JWT, ~10h TTL)
is cached in the current PowerShell session and refreshed transparently. No manual token handling needed.
...
### Known resource IDs (stage environment)
- Warehouse `RG_Claude` → `av-49jtc8yy9xi4` (Google, us-east1, 1 AU, Running)
