...
.DESCRIPTION
  ...
  Checks:
    ...
    4. JR runtime jars: Resolve-JrLib + a jasperreports-*.jar and the PostgreSQL
       driver jar in that dir.
    5. Chart-customizer jar on the live JRS classpath (WARN if absent -- only
       bar-gradient reports need it).
    6. Python on PATH; sqlglot + pypdfium2 importable (WARN if missing).
    7. Key scripts present in scripts\.
...
Run-Check "JRS server reachable" {
    ...
    $resp = & curl.exe -s -S -w "`n%{http_code}" -u "$($jrs.User):$($jrs.Password)" -H "Accept: application/json" $url
...
Run-Check "chart-customizer jar" {
    ...
    $hint = "absent from JRS classpath -- only bar-gradient reports need it."
...
Run-Check "Python (sqlglot, pypdfium2)" {
    ...
    if (-not $py) {
        return @{ Status = "WARN"; Detail = "python not on PATH; scaffold_*.py / lineage / baselines unavailable" }
    }
