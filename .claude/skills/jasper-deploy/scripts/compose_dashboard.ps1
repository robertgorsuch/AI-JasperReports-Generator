...
if (-not $KeepExisting) {
    ...
    if ($Backup -and ($check0 = & (Get-JrsCurl) -s -o (Get-JrsNull) -w "%{http_code}" -u $auth "$($jrs.ServerUrl)/rest_v2/resources$dashUri") -and "$check0".Trim() -match '^2\d\d$') {
    ...
    $delCode = & (Get-JrsCurl) -s -o (Get-JrsNull) -w "%{http_code}" -u $auth -X DELETE "$($jrs.ServerUrl)/rest_v2/resources$dashUri"
